...
void onStart(ServiceInstance service) async {
  ...
  try {
    ...
    selectedAzanFile =
        prefs.getString('selected_azan_file') ?? 'azan_makkah.mp3';
  } catch (e) {
    ...
    try {
      ...
      if (timingsDateStr != todayStr) {
        ...
        if (lat != null && lng != null) {
          ...
          final url =
              'https://api.aladhan.com/v1/timings/$ts?latitude=${lat.toStringAsFixed(4)}&longitude=${lng.toStringAsFixed(4)}&method=5';
          try {
            ...
            if (res.statusCode == 200) {
              ...
              final newTimings =
                  Map<String, dynamic>.from(body['data']['timings'] as Map);
  ...
  try {
    ...
    debugPrint("Failed to set background audio context: $e");
  }
  ...
  // ✅ timer واحد بس كل 30 ثانية بدل timer كل 10 ثواني
  Timer.periodic(const Duration(seconds: 30), (timer) async {
            ...
            if (rawTime != null) {
              ...
              if (parts.length >= 2) {
                ...
                if (hr != null &&
                    ...
                    now.minute == mn) {
                  final todayPrayerId =
                      "${now.year}-${now.month}-${now.day}_$key";
                  final lastPlayed =
                      prefs.getString('last_played_prayer') ?? '';
                  if (lastPlayed != todayPrayerId && !isAzaanPlaying) {
                    isAzaanPlaying = true;
                    await player.stop();
    ...
    // ✅ تحديث الـ notification كل دقيقة بس (كل دورتين من الـ timer)
...
Future<void> initializeService() async {
  ...
  await service.configure(
    androidConfiguration: AndroidConfiguration(
      ...
      autoStartOnBoot: true, // ✅ يرجع بعد restart الجهاز
    ),
...
Future<void> updateServiceState() async {
  ...
  final bool prayerEnabled =
      prefs.getBool('prayer_notification_enabled') ?? false;
  final bool azanEnabled = prefs.getBool('azan_sound_enabled') ?? true;

  final service = FlutterBackgroundService();
  final isRunning = await service.isRunning();

  // ✅ الـ service شغالة دايماً
  if (!isRunning) {
